    ...
    if (services.pluginState) {
      try {
        ...
        services.pluginState.configure({
          pluginId: 'PluginTemplate',
          stateStrategy: 'session',
          preserveKeys: ['data', 'userPreferences'],
          stateSchema: {
            data: { type: 'object', required: false, default: null },
            userPreferences: { type: 'object', required: false, default: {} }
          }
        });

        // Set up lifecycle hooks
        const onSaveUnsubscribe = services.pluginState.onSave((state) => {
          console.log('PluginTemplate: State saved:', state);
        });

        const onRestoreUnsubscribe = services.pluginState.onRestore((state) => {
          console.log('PluginTemplate: State restored:', state);
          // TODO: Apply restored state to component
        });

        const onClearUnsubscribe = services.pluginState.onClear(() => {
          console.log('PluginTemplate: State cleared');
        });
        ...
        if (restoredState) {
          console.log('PluginTemplate: Restored state:', restoredState);
          // TODO: Apply restored state to component state
        }
      } catch (error) {
        console.warn('PluginTemplate: Failed to initialize plugin state:', error);
      }
    }
  ...
  private cleanupServices(): void {
    ...
    this.pluginStateUnsubscribers.forEach(unsubscribe => {
      try {
        ...
        console.warn('PluginTemplate: Error cleaning up plugin state listener:', error);
      }
    });
          ...
          <div className="info-grid">
            ...
            <div className="info-item">
              ...
              <ul>
                ...
                <li>Plugin State: {services.pluginState ? '✅' : '❌'}</li>
              </ul>
            </div>
          </div>
        </div>
