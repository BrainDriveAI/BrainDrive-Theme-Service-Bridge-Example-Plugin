#!/usr/bin/env python3
"""
ServiceExample_Theme Plugin Lifecycle Manager (New Architecture)

This script handles install/update/delete operations for the ServiceExample_Theme plugin
...
try:
    ...
    try:
        ...
        raise ImportError("ServiceExample_Theme plugin requires the new architecture BaseLifecycleManager")


class ServiceExampleThemeLifecycleManager(BaseLifecycleManager):
    """Lifecycle manager for ServiceExample_Theme plugin using new architecture"""
    ...
    def __init__(self, plugins_base_dir: str = None):
        ...
        self.plugin_data = {
            ...
            "compatibility": "1.0.0",
            "scope": "ServiceExampleTheme",
            "bundle_method": "webpack",
            "bundle_location": "dist/remoteEntry.js",
            "is_local": False,
            "long_description": "A comprehensive example demonstrating how to use the Theme Service in BrainDrive plugins for theme management and switching. Shows theme detection, switching controls, and change event handling patterns.",
        ...
        # Define module data for the three theme components
        self.module_data = [
            {
                "name": "ThemeDisplay",
                "display_name": "Theme Display",
                "description": "Module that displays current theme information and service status",
                "icon": "Eye",
                "category": "examples",
                "priority": 1,
                "props": {
                    "title": "Theme Display",
                    "description": "Display current theme information"
                },
                "config_fields": {},
                "messages": {},
                "required_services": {
                ...
                "tags": ["theme", "display", "example"]
            },
            {
                "name": "ThemeController",
                "display_name": "Theme Controller",
                "description": "Module that provides theme switching controls",
                "icon": "Settings",
                "category": "examples",
                "priority": 2,
                "props": {
                    "title": "Theme Controller",
                    "description": "Control and switch themes"
                },
                "config_fields": {},
                "messages": {},
                "required_services": {
                ...
                "tags": ["theme", "controller", "example"]
            },
            {
                "name": "ThemeListener",
                "display_name": "Theme Listener",
                "description": "Module that demonstrates theme change event handling",
                "icon": "Activity",
                "category": "examples",
                "priority": 3,
                "props": {
                    "title": "Theme Listener",
                    "description": "Monitor and log theme changes"
                },
                "config_fields": {},
                "messages": {},
                "required_services": {
                ...
                "tags": ["theme", "listener", "monitor", "example"]
            }
        ]
        
        # Initialize base class with required parameters
        logger.info(f"ServiceExample_Theme: plugins_base_dir - {plugins_base_dir}")
        if plugins_base_dir:
        ...
        logger.info(f"ServiceExample_Theme: shared_path - {shared_path}")
        super().__init__(
    ...
    async def _perform_user_installation(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_Theme: User installation completed for {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"ServiceExample_Theme: User installation failed for {user_id}: {e}")
    ...
    async def _perform_user_uninstallation(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_Theme: User uninstallation completed for {user_id}")
            return {
                'success': True,
                'plugin_id': plugin_id,
            ...
            logger.error(f"ServiceExample_Theme: User uninstallation failed for {user_id}: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _copy_plugin_files_impl(self, user_id: str, target_dir: Path, update: bool = False) -> Dict[str, Any]:
        """
        ServiceExample_Theme-specific implementation of file copying.
        ...
        try:
            ...
            logger.info(f"ServiceExample_Theme: Copied {len(copied_files)} files/directories to {target_dir}")
            return {'success': True, 'copied_files': copied_files}
            
        except Exception as e:
            logger.error(f"ServiceExample_Theme: Error copying plugin files: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _validate_installation_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        ServiceExample_Theme-specific validation logic.
        This method is called by the base class during installation.
        """
        try:
            # Check for ServiceExample_Theme-specific required files
            ...
            if missing_files:
                return {
                    'valid': False,
                    'error': f"ServiceExample_Theme: Missing required files: {', '.join(missing_files)}"
                }
            ...
            try:
                ...
                for field in required_fields:
                    if field not in package_data:
                        return {
                            'valid': False,
                            'error': f'ServiceExample_Theme: package.json missing required field: {field}'
                        }
                        
            except (json.JSONDecodeError, FileNotFoundError) as e:
                return {
                    'valid': False,
                    'error': f'ServiceExample_Theme: Invalid or missing package.json: {e}'
                }
            ...
            if bundle_path.stat().st_size == 0:
                return {
                    'valid': False,
                    'error': 'ServiceExample_Theme: Bundle file (remoteEntry.js) is empty'
                }
            
            logger.info(f"ServiceExample_Theme: Installation validation passed for user {user_id}")
            return {'valid': True}
            
        except Exception as e:
            logger.error(f"ServiceExample_Theme: Error validating installation: {e}")
            return {'valid': False, 'error': str(e)}
    
    async def _get_plugin_health_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        ServiceExample_Theme-specific health check logic.
        ...
        try:
            ...
            logger.error(f"ServiceExample_Theme: Error checking plugin health: {e}")
            return {
                'healthy': False,
                'details': {'error': str(e)}
            }
    ...
    async def _check_existing_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            plugin_slug = self.plugin_data['plugin_slug']
            logger.info(f"ServiceExample_Theme: Checking for existing plugin - user_id: {user_id}, plugin_slug: {plugin_slug}")
            
            # First test database connectivity
            test_query = text("SELECT COUNT(*) as count FROM plugin")
            test_result = await db.execute(test_query)
            test_row = test_result.fetchone()
            logger.info(f"ServiceExample_Theme: Database connectivity test - total plugins: {test_row.count}")
            ...
            logger.info(f"ServiceExample_Theme: Executing query with params: {query_params}")
            
            result = await db.execute(plugin_query, query_params)
            
            plugin_row = result.fetchone()
            logger.info(f"ServiceExample_Theme: Query result: {plugin_row}")
            if plugin_row:
                logger.info(f"ServiceExample_Theme: Found existing plugin - id: {plugin_row.id}, name: {plugin_row.name}")
                return {
                    'exists': True,
                ...
                logger.warning(f"ServiceExample_Theme: No plugin found for user_id: {user_id}, plugin_slug: {plugin_slug}")
                ...
                if debug_rows:
                    logger.info(f"ServiceExample_Theme: User has {len(debug_rows)} other plugins:")
                    for row in debug_rows:
                        logger.info(f"  - {row.plugin_slug} (id: {row.id})")
                else:
                    logger.info(f"ServiceExample_Theme: User has no plugins installed")
                
                return {'exists': False}
                
        except Exception as e:
            logger.error(f"ServiceExample_Theme: Error checking existing plugin: {e}")
    ...
    async def _create_database_records(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_Theme: Creating database records - user_id: {user_id}, plugin_slug: {plugin_slug}, plugin_id: {plugin_id}")
            ...
            await db.commit()
            logger.info(f"ServiceExample_Theme: Database transaction committed successfully")
            ...
            if verify_row:
                logger.info(f"ServiceExample_Theme: Successfully created and verified database records for plugin {plugin_id} with {len(modules_created)} modules")
            else:
                logger.error(f"ServiceExample_Theme: Plugin creation appeared to succeed but verification failed for plugin_id: {plugin_id}")
    ...
    async def _export_user_data(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_Theme: Exported user data for {user_id}")
            return {'success': True, 'user_data': user_data}
            
        except Exception as e:
            logger.error(f"ServiceExample_Theme: Error exporting user data: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _import_user_data(self, user_id: str, db: AsyncSession, user_data: Dict[str, Any]):
        ...
        try:
            if not user_data:
                logger.info(f"ServiceExample_Theme: No user data to import for {user_id}")
                return
            ...
            logger.info(f"ServiceExample_Theme: Imported user data for {user_id}")
            
        except Exception as e:
            logger.error(f"ServiceExample_Theme: Error importing user data: {e}")
            raise
    ...
    async def install_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Install ServiceExample_Theme plugin for specific user (compatibility method)"""
        try:
            logger.info(f"ServiceExample_Theme: Starting installation for user {user_id}")
            
            # Check if plugin is already installed for this user
            existing_check = await self._check_existing_plugin(user_id, db)
            if existing_check['exists']:
                logger.warning(f"ServiceExample_Theme: Plugin already installed for user {user_id}")
                return {
                    'success': False,
            ...
            logger.info(f"ServiceExample_Theme: Created shared directory: {shared_path}")

            # Copy plugin files to the shared directory first
            copy_result = await self._copy_plugin_files_impl(user_id, shared_path)
            if not copy_result['success']:
                logger.error(f"ServiceExample_Theme: File copying failed: {copy_result.get('error')}")
                return copy_result

            logger.info(f"ServiceExample_Theme: Files copied successfully, proceeding with database installation")
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    if not verify_check['exists']:
                        logger.error(f"ServiceExample_Theme: Installation appeared successful but verification failed")
                        return {'success': False, 'error': 'Installation verification failed'}
                    
                    logger.info(f"ServiceExample_Theme: Installation verified successfully for user {user_id}")
                    result.update({
                        'plugin_slug': self.plugin_data['plugin_slug'],
                        'plugin_name': self.plugin_data['name']
                    })
                else:
                    logger.error(f"ServiceExample_Theme: Database installation failed: {result.get('error')}")
                
                return result
                
            except Exception as db_error:
                logger.error(f"ServiceExample_Theme: Database operation failed: {db_error}")
            ...
            logger.error(f"ServiceExample_Theme: Install plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    
    async def delete_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Delete ServiceExample_Theme plugin for user (compatibility method)"""
        try:
            logger.info(f"ServiceExample_Theme: Starting deletion for user {user_id}")
            ...
            if result.get('success'):
                logger.info(f"ServiceExample_Theme: Successfully deleted plugin for user {user_id}")
            else:
                logger.error(f"ServiceExample_Theme: Deletion failed: {result.get('error')}")
            
            return result
        except Exception as e:
            logger.error(f"ServiceExample_Theme: Delete plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    
    async def get_plugin_status(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Get current status of ServiceExample_Theme plugin installation (compatibility method)"""
        try:
            existing_check = await self._check_existing_plugin(user_id, db)
            if not existing_check['exists']:
            ...
            logger.error(f"ServiceExample_Theme: Error checking plugin status: {e}")
            return {'exists': False, 'status': 'error', 'error': str(e)}
    
    async def update_plugin(self, user_id: str, db: AsyncSession, new_version_manager: 'ServiceExample_ThemeLifecycleManager') -> Dict[str, Any]:
        """Update ServiceExample_Theme plugin for user (compatibility method)"""
        try:
            ...
            logger.info(f"ServiceExample_Theme: Plugin updated successfully for user {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"ServiceExample_Theme: Plugin update failed for user {user_id}: {e}")
            return {'success': False, 'error': str(e)}
...
async def install_plugin(user_id: str, db: AsyncSession, plugins_base_dir: str = None) -> Dict[str, Any]:
    manager = ServiceExample_ThemeLifecycleManager(plugins_base_dir)
...
async def update_plugin(user_id: str, db: AsyncSession, new_version_manager: 'ServiceExample_ThemeLifecycleManager', plugins_base_dir: str = None) -> Dict[str, Any]:
    old_manager = ServiceExample_ThemeLifecycleManager(plugins_base_dir)
...
if __name__ == "__main__":
    ...
    async def main():
        print("ServiceExample_Theme Plugin Lifecycle Manager - Test Mode")
        print("=" * 50)
        
        # Test manager initialization
        manager = ServiceExample_ThemeLifecycleManager()
